import {
  ...
  export default function AddEditDevotionalProviders({ pageMode = 'add' }) {
    ...
    const onSubmit = ({ name, position, package_name, intent_url,image }) => {
      ...
      const apiCall =
        pageMode === 'add'
          ? apis.addDEVOTIONALProviders(data)
          : apis.editDEVOTIONALProviders(location.state.state.data?.sno, data);
      ...
      apiCall
        .then(res => {
          ...
          history.push('/DevotionalProviders');
        })
        .catch(err => {
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Typography component="h1" variant="h5" className={classes.title}>
                  {pageMode === 'add' ? 'ADD' : 'EDIT'} DEVOTIONAL PROVIDERS
                </Typography>
              </Grid>
