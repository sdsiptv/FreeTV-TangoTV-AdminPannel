...
export default function AddEditAppTVProviders({ pageMode = 'add' }) {
  ...
  const onSubmit = ({ name }) => {
    ...
    const apiCall =
      pageMode === 'add'
        ? apis.addAppTVProviders(data)
        : apis.editAppTVProviders(location.state.state.data?.providerId, data);
          ...
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <Typography component="h1" variant="h5" className={classes.title}>
                {pageMode === 'add' ? 'ADD' : 'EDIT'} APP TV PROVIDERS
              </Typography>
            </Grid>
