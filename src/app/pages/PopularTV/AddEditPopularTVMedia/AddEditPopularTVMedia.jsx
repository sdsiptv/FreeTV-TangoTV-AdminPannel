...
export default function AddEditPopularTVMedia({ pageMode = 'add' }) {
  ...
  const handleLogoChange = (event) => {
    const logoUploaded = event.target.files[0];
    setLogoObj(logoUploaded);
  };
  ...
  const onSubmit = ({
    ...
    const apiCall =
      pageMode === 'add'
        ? apis.addPopularTVMedia(data)
        : apis.editPopularTVMedia(location.state.state.data?.populartv_id, data);
    apiCall
      .then(res => {
        toastMessage(
          pageMode === 'add' ? `Successfully Added ` : `Successfully updated`,
        );
        history.push('/PopularTVMedia');
      })
      .catch(error => {
        ...
        if (error.response && error.response.status) {
          ...
          if (statusCode === 400) {
            failureNotification('Popular TV Not inserted');
          ...
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <Typography component="h1" variant="h5" className={classes.title}>
                {pageMode === 'add' ? 'ADD' : 'EDIT'} POPULAR TV MEDIA
              </Typography>
            </Grid>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                helperText={"Channel ID will be Only in Numbers"}
            ...
            <Grid item xs={6}>
              <TextField
                ...
                helperText={"TMDB ID will be Only in Numbers"}
            ...
            <Grid item xs={6}>
              ...
              <img
                src={typeof logoObj === 'object' ? URL.createObjectURL(logoObj) : logo}
                alt=""
                style={{ paddingLeft: '10px', width: '100px' }}
              />
            </Grid>
