...
export default function AddEditEducationCategories({ pageMode = 'add' }) {
  ...
  useEffect(() => {
    if (location.state) {
      ...
      setValue('categoryname', params.categoryname);
  ...
  const onSubmit = data => {
    const apiCall =
      pageMode === 'add'
        ? apis.addEDUCATIONCategory(data.categoryname, data.position, data.enable)
        : apis.editEDUCATIONCategory(data.category_id, data.categoryname, data.position, data.enabled);

    apiCall
      .then(res => {
        toastMessage('Successfully added');
        history.push('/EDUCategory');
      })
      .catch(err => {
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Typography component="h1" variant="h5" className={classes.title}>
                {pageMode === 'add' ? 'Add' : 'Edit'} Education Category
              </Typography>
            </Grid>
