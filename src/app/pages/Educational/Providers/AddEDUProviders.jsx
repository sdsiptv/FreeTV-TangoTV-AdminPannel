...
export default function AddEditEDUCATIONProviders({ pageMode = 'add' }) {
  ...
  const onSubmit = ({ name, position, package_name, intent_url,image }) => {
    ...
    const apiCall =
      pageMode === 'add'
        ? apis.addEDUCATIONProviders(data)
        : apis.editEDUCATIONProviders(location.state.state.data?.sno, data);
          ...
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <Typography component="h1" variant="h5" className={classes.title}>
                {pageMode === 'add' ? 'ADD' : 'EDIT'} EDUCATION PROVIDERS
              </Typography>
            </Grid>
