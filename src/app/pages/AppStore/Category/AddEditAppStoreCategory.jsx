...
export default function AddEditAppStoreCategories({ pageMode = 'add' }) {
  ...
  const onSubmit = ({ name,position, category_id }) => {
    const apiCall =
      pageMode === 'add'
        ? apis.addAppStoreCategory(name,position)
        : apis.editAppStoreCategory( category_id,name,position);
          ...
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <Typography component="h1" variant="h5" className={classes.title}>
                {pageMode === 'add' ? 'ADD' : 'EDIT'} APP STORE CATEGORIES
              </Typography>
            </Grid>
